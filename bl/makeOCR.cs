    ...
    public class MakeOCR
    {
        ...
        public string getOCR(byte[] imageBytes, MemoryStream memoryStream)
        {
            ...
            try
            {
                ...
                string dllPathCvExtern = Path.Combine(basePath, "cvextern.dll");
                string dllPathConcrt140 = Path.Combine(basePath, "concrt140.dll");
                string dllPathMsvcp140 = Path.Combine(basePath, "msvcp140.dll");
                string dllPathmsvcp140_1 = Path.Combine(basePath, "msvcp140_1.dll");
                string dllPathmsvcp140_2 = Path.Combine(basePath, "msvcp140_2.dll");
                string dllPathmsvcp140_atomic = Path.Combine(basePath, "msvcp140_atomic_wait.dll");
                string dllPathmsvcp140_codec = Path.Combine(basePath, "msvcp140_codecvt_ids.dll");
                string dllPathOpenvideo = Path.Combine(basePath, "opencv_videoio_ffmpeg480_64.dll");
                string dllPathVcruntime140 = Path.Combine(basePath, "vcruntime140.dll");
                string dllPathVcruntime140_1 = Path.Combine(basePath, "vcruntime140_1.dll");

                File.Copy(dllPathCvExtern, Path.Combine(basePath, "bin", "cvextern.dll"), true);

                string relativePath = "libs";
                string fullPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, relativePath);
                //if(Environment == "")
                var envi = Environment.GetEnvironmentVariables();
                Environment.SetEnvironmentVariable("PATH", Environment.GetEnvironmentVariable("PATH") + ";" + fullPath);
