...
namespace APIRPA.Controllers
{
    ...
    {
        [HttpGet]
        [Route("api/GetTodayLogs")]
        public string GetTodayLogs()
        {
            //DateTime.Now:yyyy - MM - dd_HH - mm - ss
            var fechaHoy = DateTime.Now.ToString("yyyy-MM-dd");
            var logsInDirectory = Directory.GetFiles(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Logs"));
            var filteredLogs = new List<string>();
            // Recorro todos los logs que hay en la carpeta
            // ? puede devolver la lista de
            foreach (var log in logsInDirectory)
            {
                var nombre = Path.GetFileName(log);

                if (nombre.Contains(fechaHoy))
                {
                    filteredLogs.Add(nombre);
                }
            }

            return string.Join("//  //", filteredLogs);
        }

        [HttpGet()]
        [Route("api/GetLogByName")]
        public string GetLogByName(string name)
        {
            //var fechaHoy = DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss");
            var logsInDirectory = Directory.GetFiles(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Logs"));
            foreach (var log in logsInDirectory)
            {
                var nombre = Path.GetFileName(log);
                ...
                {
                    return Convert.ToString(File.ReadAllText(log));
                }                  
            }
            return "no hay ningun log";
        }
        ...
        public HttpResponseMessage cambioPassBD([FromBody] FormularioPassModel entradaDNI, string cambioPass)
        {
            ...
            // SI NO CAMBIAMOS EL TIPO DE STATUS, SE INICIALIZA CON STATUS 200
            // POSIBLEMENTE POR ESO SIEMPRE RECIBIAMOS UN STATUS 200 AUN SIN CONECTAR LA BASE DE DATOS
            HttpResponseMessage response = new HttpResponseMessage();
            // CAMBIAMOS LO PRIMERO EL ESTADO PARA QUE SI SIGUE EN UNAUTHORIZED ES QUE NO SE HA HECHO NADA
            ...
            if (cambioPass == "1")
            {
                ...
                try
                {
                    ...
                    else // ! OCRFOUND EXISTS
                    {
                        ...
                        Logger.Instance.Log("Inicio Obtención de MRZ y verificación");
                        mrzIsOk = mrzDetector.getMrzAndVerify(ocrFound);
                        ...
                        if (mrzIsOk)
                        {
                            ...
                            varNewPass = reseteoPassword.genPass();
                            Logger.Instance.Log("Contraseña nueva creada");
                            // ASIGNAMOS LOS DATOS OBTENIDOS A LOS PARAMETROS NECESARIOS PARA INICIAR EL PROCESO DE LA BBDD
                            ...
                            Logger.Instance.Log("Datos asignados a los parametros para la llamada a la bbdd");
                            ...
                            Logger.Instance.Log("BdResponse: " + bdResponse);
                            ...
                            if (bdResponse == null)
                            {
                                ...
                                response.Content = new StringContent("NO HA HABIDO RESPUESTA POR PARTE DE LA BBDD");
                                // DEBUG EN PRODUCCION
                                entradaDNI.errorCallCambioPassBD = bdResponse;
                            }
                            else
                            {
                                switch (bdResponse.ToString())
                                {
                                    case "0":
                        ...
                        else // ! MRZCREATED = NULL
                        {
                            ...
                            // DEBUG EN PRODUCCION
                            entradaDNI.errorCallCambioPassBD = mrzIsOk.ToString();
                        } // x if (mrzCreated != null)                    
                    } // x if (ocrFound.IsNullOrWhiteSpace() || ocrFound == "")
                }
                catch (Exception e)
